package com.example.bchan92.homework1.service;
...
import com.example.bchan92.homework1.network.CacheUtils;
import com.example.bchan92.homework1.network.NetworkUtils;
...
public class NetworkIntentService extends IntentService {
    ...
    private void handleActionLoad() {
        ...
        try {
            ...
            if (values != null) {
                getContentResolver().bulkInsert(CurrentConditionsContract.URI, values);
...
//                        + formatter.format(calendar.getTime()) + "'").build();
        ...
        // http://api.openweathermap.org/data/2.5/weather?id=5809844&units=imperial&APPID=8daa38a1b54d0ef6b677b13b1ec828df
        Uri uri = new Uri.Builder()
                .scheme("http")
                ...
                .appendQueryParameter("APPID", "8daa38a1b54d0ef6b677b13b1ec828df").build();
    ...
    private ContentValues processObject(JsonReader jsonReader, SimpleDateFormat formatter) throws IOException {
        ...
        while (jsonReader.hasNext()) {
            ...
            switch (name) {
                case CurrentConditionsContract.Json.CURRENT_CONDITION:
                    values.put(CurrentConditionsContract.Columns.CURRENT_CONDITION, jsonReader.nextString());
                    break;
                case CurrentConditionsContract.Json.CURRENT_TEMP:
                    values.put(CurrentConditionsContract.Json.CURRENT_TEMP, jsonReader.nextDouble());
                    break;
                case CurrentConditionsContract.Json.HUMIDITY:
                    values.put(CurrentConditionsContract.Json.HUMIDITY, jsonReader.nextLong());
                    break;
                case CurrentConditionsContract.Json.WIND_SPEED:
                    values.put(CurrentConditionsContract.Json.WIND_SPEED, jsonReader.nextDouble());
                    break;
                case CurrentConditionsContract.Json.CITY_NAME:
                    values.put(CurrentConditionsContract.Json.CITY_NAME, jsonReader.nextString());
                    break;
                default:

...
public class CursorUtilities {
    ...
    public static void validateCursor(String error, Cursor valueCursor, ContentValues[] valuesArray) {
        ...
        while (valueCursor.moveToNext()) {
            String name = valueCursor.getString(valueCursor.getColumnIndex(ContactContract.Columns.NAME));
            for (ContentValues values : valuesArray) {
                if (values.getAsString(ContactContract.Columns.NAME).equals(name)) {
                    ++valueCount;
